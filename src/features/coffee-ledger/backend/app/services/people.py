...
def register_person(conn: psycopg.Connection, name: str | None) -> dict[str, Any]:
    ...
    log.info("人の登録成功 person_id=%s name=%s display_order=%s", row["id"], row["name"], row["display_order"])
...
def deactivate_person(conn: psycopg.Connection, person_id: int) -> dict[str, Any]:
    log.info("利用停止要求 person_id=%s", person_id)
    ...
    log.info("利用停止成功 person_id=%s name=%s", updated["id"], updated["name"])
...
def update_display_order(
    ...
    log.info("表示順変更要求 person_ids=%s", person_ids)
    if not person_ids:
        raise validation_error()
    ...
    log.info("表示順変更成功 person_ids=%s", person_ids)
