...
- トランザクション: 1 要求 = 1 トランザクション。応答が 400 以上の場合は、途中の変更（操作記録を含む）をすべて取り消す。ただし `POST /maintenance/vacuum` はトランザクションの外で実行する（台帳のデータは変えない）。
- CORS: `CORS_ORIGINS` に書いたオリジンだけを許可する。資格情報（Cookie）は許可しない。CSV のファイル名をフロントが読めるよう、`Content-Disposition` ヘッダを公開する。
- 応答の形式は JSON。ただし `GET /summary/csv` の成功時だけ CSV を返す（エラー時は JSON）。
...
| GET | /summary/csv | 不要 | 金銭の動きの CSV 出力 | REQ-023 |
| GET | /operation-logs | 不要 | 操作記録の一覧 | REQ-018 |
| POST | /maintenance/vacuum | 不要 | DB の整理 | REQ-022 |
...
- **認証**: 不要
- **要求**: なし
- **応答（200）**: 金銭の動きの CSV。出来事と積み上げの値は `GET /summary` の `entries` と同じで、並びは古い順。

| ヘッダ | 値 |
|--------|----|
| Content-Type | `text/csv; charset=utf-8` |
| Content-Disposition | `attachment; filename="coffee-ledger-YYYYMMDD-HHmmss.csv"`（要求を受けた日本時間） |
| Cache-Control | `no-store` |

本文は UTF-8（先頭に BOM）、改行は CRLF。値にカンマ・引用符・改行があれば引用符で囲み、引用符は二重にする。

```csv
日時,種類,名前,金額,入出金,事由,事由の日付,修正前の額,修正後の額,未徴収,徴収済み,金庫
2026-09-24 09:15:00,飲む,山田,100,,,,,,100,0,0
2026-09-24 12:00:00,支払,山田,100,,,,,,0,100,0
2026-09-24 18:00:00,金庫収納,,100,,,,,,0,0,100
2026-09-24 18:05:00,金庫操作,,60,出金,"豆の購入, 500g",2026-09-24,,,0,0,40
2026-09-24 18:10:00,未払い修正,山田,,,端数の調整,,0,20,20,0,40
```

| 列 | 内容 |
|----|------|
| 日時 | 出来事の日時（日本時間 `YYYY-MM-DD HH:mm:ss`。`GET /summary` の `occurred_at` と同じ時点） |
| 種類 | 飲む / 飲用の取り消し / 支払 / 支払の取り消し / 金庫収納 / 金庫操作 / 未払い修正 |
| 名前 | 人の現在の名前（飲用・支払・未払い修正のみ） |
| 金額 | 単価・支払額・収納額・操作額（未払い修正は空） |
| 入出金 | 入金 / 出金（金庫操作のみ） |
| 事由 | 金庫操作・未払い修正のみ |
| 事由の日付 | `YYYY-MM-DD`（金庫操作のみ） |
| 修正前の額 / 修正後の額 | 未払い修正のみ |
| 未徴収 / 徴収済み / 金庫 | 出来事の直後の金額 |

金額はすべて区切りなしの整数。出来事が無いときは見出しの行だけを返す。操作記録には残さない。

- **エラー**: 共通エラーのみ（JSON で返す）
...
- **認証**: 不要
- **要求**: 本文なし。
- **応答（200）**: 台帳の全テーブルに `VACUUM (ANALYZE)` を実行した結果。`elapsed_seconds` は所要時間（秒、小数第 1 位まで）。
...
トランザクションの外で実行する。台帳のデータは変えず、操作記録にも残さない。

- **エラー**

| HTTP ステータス | 条件 |
|------|------|
| 409 VACUUM_IN_PROGRESS | 別の DB の整理を実行中。「DB の整理を実行中です。しばらくしてからやり直してください。」 |
| 500 INTERNAL_ERROR | DB の整理に失敗した（共通エラー） |
...
| 2026-09-24 15:01 | 未承認 | `GET /summary/csv`（REQ-023）と `POST /maintenance/vacuum`（REQ-022）を追加。共通事項に CSV 応答・ヘッダ公開・トランザクションの例外を追記 |
| 2026-09-24 15:03 | 承認済み | CSV 出力と DB の整理のエンドポイント追加を承認 |
