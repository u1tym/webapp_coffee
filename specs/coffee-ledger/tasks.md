...
### 変更（REQ-003 完了モーダルのカウントダウン）

| No | タスク | 対応する要件/設計 | 実装パス | 所要時間 | 完了条件 |
|----|--------|--------------------|----------|----------|----------|
| T-029 | SCR-001 の完了モーダルの文言を「毎度ありがとうございます」に変え、その下にカウントダウン（5〜1）を表示して、5 秒後に自動で閉じるようにする | REQ-003 / ui-design.md「SCR-001（完了モーダル）」 | `<cl>/frontend/src/views/HomeView.vue` | 1h | 飲むを押すと「毎度ありがとうございます」と「5」が出て、1 秒ごとに 4、3、2、1 と減り、1 の 1 秒後にモーダルが閉じて一覧を操作できるようになる。途中で OK を押すとすぐに閉じ、カウントダウンも止まる（閉じた後に次の飲用を記録しても、前のカウントダウンの影響で早く閉じない）。モーダルが開いている間は一覧のボタンが無効。画面を離れたときにタイマーが残らない。`npm run build` が型エラーなしで通る |
...
| 2026-09-25 17:24 | 未承認 | REQ-003 の変更（完了モーダルの文言とカウントダウン）に対応するタスク T-029 を追加 |
| 2026-09-25 17:24 | 承認済み | T-029 の追加を承認 |
